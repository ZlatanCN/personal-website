...
tags: ['JavaScript']
draft: false
summary: 本文深入讲解了 JavaScript 中闭包 (Closure) 的工作原理，重点围绕词法环境 (Lexical Environment) 和作用域链的构建过程。通过详细的执行步骤说明，展示了函数如何“记住”其创建时的环境，并在后续调用时依然能够访问该环境中的变量。
---
...
> [!tip] 一个比喻
>
> `[[Environment]]` 是一张只告诉你“宝藏在A村”的藏宝图。而 `[[Scopes]]` 是浏览器拿到这张图后，帮你把“你当前位置 → A村 → A村所属的省 → ... → 全世界”的完整路线图都画了出来。
