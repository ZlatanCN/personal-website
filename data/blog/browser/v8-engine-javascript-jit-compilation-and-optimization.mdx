...
隐藏类的核心理念是为具有相同“结构”（或称为“形状”）的对象共享一个内部蓝图，从而提升属性访问效率。这个机制带来了两大互相促进的核心优势：

1. **将动态查找转换为静态偏移量访问（加速访问）**：这是隐藏类的**直接目的**。当 V8 确认一组对象共享同一个隐藏类 (Hidden Class) 时，它就知道这些对象在内存中的布局是完全一致的。此时，V8 会在首次访问属性（如 `obj.y`）时，通过一次慢速查找确定其在隐藏类中的**内存偏移量 (offset)**，并将该结果缓存在**内联缓存 (Inline Caching)** 中；后续对相同隐藏类的对象的同属性访问时，即可直接复用这个偏移量，一步到位找到属性值，从而让动态语言的属性访问速度逼近静态编译语言。

![Hidden Class 示例](/static/images/v8-engine-javascript-jit-compilation-and-optimization/hidden-class.png)

> [!note] 偏移量缓存
> 上图的 `person2[0].indexOf('name')` 会被缓存为 `1`。

2. **为推测性优化提供形状判断依据（确定形状是否相同）**：这是隐藏类的**间接收益**，也是 JIT 编译的关键。当 TurboFan 准备对一个函数（例如 `function process(obj) {...}`）进行优化时，它需要判断这个函数是否处于“单态 (Monomorphism)”，即传入的 `obj` 是否总是具有相同的形状。得益于隐藏类机制，这个判断变得极其高效：**V 8 只需要比较前后传入的几个 `obj` 的隐藏类地址是否相同即可**。如果地址始终一致，TurboFan 就可以大胆地进行推测性优化，生成一段专门处理该特定形状对象的高速机器码。
...
> [!tip] 优化隐藏类 (Hidden Classes) 的建议  
> 为确保 V8 引擎的隐藏类 (Hidden Classes) 稳定并最大化 JIT 编译器的优化效果，建议在对象创建时预定义**所有**可能使用的属性（即使暂时设为 `undefined`），并保持属性添加的顺序一致。同时，避免动态添加或删除属性，以维持对象形状的稳定性，从而提升属性访问速度和代码执行效率。
...
作为开发者，我们可以基于对代码的理解，预测某些小型函数是否会发生改变。如果函数逻辑简单且稳定，手动内联可能是一个可行的优化选择，尤其在性能敏感场景（如实时系统或高频调用）下，可以精确控制代码执行路径，减少函数调用开销。

> [!caution] 手动内联需谨慎
> 手动内联可能增加代码重复和维护成本，降低可读性，且难以复现编译器的复杂优化（如常量折叠）。建议在性能测试验证后，仅对小型、稳定函数进行手动内联，并通过注释或工具管理维护负担。
